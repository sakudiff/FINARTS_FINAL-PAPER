...
This daily restricted grid corresponds to the paper's Figures 4 through 8 and Appendix Figure A3.1. Five variables match the paper. Risk_off declines from approximately 0.10 to 0 in the paper and from 0.098 to 0 in our replication. SPREAD peaks at 0.010 at day 50 in the paper and at 0.015 at day 50 in our data, then decays to 0.007 and 0.012. RGDP troughs at -0.0006 at day 45 in the paper and at -0.00074 at day 50 in our data. Both confidence intervals stay below zero. REER reaches a +0.0008 plateau in the paper and a +0.002 plateau in our data. DIRECT dips to -0.002 in the paper and reaches +0.0015 at day 50 in our data, settling at +0.0009. Three variables are partial matches. STOCK shows a +0.0005 blip then a decline to -0.001 in the paper. Our estimate declines from -0.0014 to -0.0066 with no initial blip. DEBTSEC dips to -0.005 then flattens at zero in the paper. Our estimate is noisy and settles near zero. EQUITY troughs at -0.006 at day 40 in the paper and recovers to +0.001. Our estimate troughs at -0.038 at day 25 and recovers to -0.0017. Two variables are mismatches. WUI rises to a +0.0017 plateau in the paper. Our estimate troughs at -0.01 at day 40 and returns to zero. OTHER peaks at +0.010 at day 50 in the paper. Our estimate spikes to +0.118 at day 5 then reaches -0.017 at day 50.
...
This daily unrestricted grid corresponds to the paper's Appendix Figure A2.1. The unrestricted specification removes the block exogeneity constraint. The cross-validation values in section 6 apply to the restricted specification only. The unrestricted results show the same directional patterns with wider confidence bands. The daily RGDP response remains negative and persistent.
...
This monthly restricted grid corresponds to the paper's A4 monthly panels in grid form. Four variables match the paper. REER rises from +0.004 to a peak of +0.005 at months 3 to 4 in the paper, ending at -0.001. Our estimate rises from +0.005 to +0.012 at month 3, ending at +0.0008. STOCK starts at -0.017 in the paper and recovers to -0.004 at month 40. Our estimate starts at -0.021 and recovers to +0.0003. SPREAD starts at 0.04 in the paper and 0.024 in our data, spikes to 0.075 and 0.115 at month 3, then decays to 0.01 and 0.003. EQUITY troughs at -0.065 at month 4 in the paper and rebounds to +0.025. Our estimate troughs at -0.055 at month 3 and rebounds to +0.026. Two variables are partial matches. RGDP starts at +0.001 in the paper and at -0.00004 in our data. It troughs at -0.002 at months 7 to 8 in the paper and at -0.004 at month 3 in our data. Both recover toward zero by month 40. DEBTSEC starts at +0.06 in the paper and at -0.15 in our data. Both are volatile and settle near zero. One variable is a mismatch. WUI starts at +0.002 in the paper and dips to -0.003 at month 7. Our estimate peaks at +0.052 at month 5 and oscillates.
...
This monthly unrestricted grid is a robustness check. It removes the block exogeneity restriction from the monthly specification. The unrestricted results diverge from the restricted specification on current-vintage data. The paper states the two are consistent. This divergence is itself a finding about model sensitivity to GDP revisions.
...
This daily restricted grid extends the sample to 2026. It uses current-vintage data for all series. The extended period RGDP confidence intervals are wider due to post-2022 GDP benchmark revisions. The directional responses are consistent with the paper period.
...
This daily unrestricted grid extends the unrestricted specification to 2026 on current-vintage data. It extends the analysis of the paper's Appendix Figure A2.1 beyond the original sample.
...
This monthly restricted grid extends the monthly restricted VAR to 2026. The extended period uses current-vintage data for all series. The RGDP response remains negative through the first six months, consistent with the paper period.
...
This monthly unrestricted grid extends the unrestricted monthly specification to 2026. It shows the unrestricted model over the full 1999 to 2026 sample.

```{r repl-fig-panel-wui}
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/irf_panel_wui_paper.png")
```

This daily and monthly frequency panel corresponds to the paper's Appendix Figure A4.1. WUI is a mismatch at both frequencies. The paper's daily WUI rises to a +0.0017 plateau. Our daily estimate troughs at -0.01 at day 40. The paper's monthly WUI starts at +0.002 and dips to -0.003 at month 7. Our monthly estimate peaks at +0.052 at month 5 and oscillates. The paper's own figures show this frequency dependence: daily WUI reaches +0.0017 and monthly WUI reaches -0.003. Our replication reproduces the sign instability but not the magnitude or the smooth pattern.

```{r repl-fig-panel-rgdp}
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/irf_panel_rgdp_paper.png")
```

This panel corresponds to the paper's Appendix Figure A4.2. RGDP is a match at daily frequency and a partial match at monthly frequency. At daily frequency the paper troughs at -0.0006 at day 45. Our estimate troughs at -0.00074 at day 50. Both confidence intervals stay below zero. At monthly frequency the paper reaches -0.002 at months 7 to 8. Our estimate troughs at -0.004 at month 3. Both recover toward zero.

```{r repl-fig-panel-reer}
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/irf_panel_reer_paper.png")
```

This panel corresponds to the paper's Appendix Figure A4.3. REER matches the paper at both frequencies. At daily frequency the paper reaches a +0.0008 plateau. Our estimate reaches a +0.002 plateau. At monthly frequency the paper peaks at +0.005 at months 3 to 4. Our estimate peaks at +0.012 at month 3.

```{r repl-fig-panel-spread}
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/irf_panel_spread_paper.png")
```

This panel corresponds to the paper's Appendix Figure A4.4. SPREAD matches the paper at both frequencies. At daily frequency the paper peaks at 0.010 at day 50 and decays to 0.007. Our estimate peaks at 0.015 at day 50 and decays to 0.012. At monthly frequency the paper spikes to 0.075 at month 3 and decays to 0.01. Our estimate spikes to 0.115 at month 3 and decays to 0.003.

```{r repl-fig-panel-stock}
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/irf_panel_stock_paper.png")
```

This panel corresponds to the paper's Appendix Figure A4.5. The Nikkei 225 response is a partial match at daily frequency and a match at monthly frequency. At daily frequency the paper shows a +0.0005 blip then a decline to -0.001. Our estimate declines from -0.0014 to -0.0066 with no initial blip. At monthly frequency the paper starts at -0.017 and recovers to -0.004 at month 40. Our estimate starts at -0.021 and recovers to +0.0003.

```{r repl-fig-panel-debtsec}
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/irf_panel_debtsec_paper.png")
```

This panel corresponds to the paper's Appendix Figure A4.6. DEBTSEC is a partial match at both frequencies. At daily frequency the paper dips to -0.005 then flattens at zero. Our estimate is noisy and settles near zero. At monthly frequency the paper starts at +0.06, is volatile, and settles to zero. Our estimate starts at -0.15, is volatile, and settles to zero. The directional paths differ but both converge to zero.

```{r repl-fig-panel-equity}
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/irf_panel_equity_paper.png")
```

This panel corresponds to the paper's Appendix Figure A4.7. EQUITY is a partial match at daily frequency and a match at monthly frequency. At daily frequency the paper troughs at -0.006 at day 40 and recovers to +0.001. Our estimate troughs at -0.038 at day 25 and recovers to -0.0017. At monthly frequency the paper troughs at -0.065 at month 4 and rebounds to +0.025. Our estimate troughs at -0.055 at month 3 and rebounds to +0.026.
...
crossval <- data.frame(
  ...
  Our_value = c(
    "0.098 to 0 by day 50",
    "trough -0.01 day 40, returns to 0",
    "peak 0.015 day 50, decays to 0.012",
    "trough -0.00074 day 50, at -0.00064 day 125",
    "+0.002 plateau",
    "-0.0014 to -0.0066, no blip",
    "noisy, settles near 0",
    "-0.038 day 25, recovers to -0.0017",
    "+0.118 day 5, then -0.017 day 50",
    "+0.0015 day 50, +0.0009",
    "+0.052 peak month 5, oscillates",
    "-0.00004 start, trough -0.004 month 3, -0.0004 month 40",
    "+0.005, peak +0.012 month 3, ends +0.0008",
    "-0.021 start, recovers to +0.0003 month 40",
    "0.024 start, 0.115 spike month 3, decays to 0.003",
    "-0.15 start, volatile, settles to 0",
    "-0.055 trough month 3, +0.026 rebound month 5"),
