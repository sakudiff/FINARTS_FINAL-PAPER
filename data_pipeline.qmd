...
system2("python3", c("scripts/quadratic_interpolate.py"))
```
...
ro_reer <- reer |>
  left_join(daily_fin |> select(date, risk_off), by = "date") |>
  filter(risk_off == 1) |>
  mutate(ep = cumsum(c(1, diff(date) > 10))) |>
  group_by(ep) |>
  summarise(start = min(date), end = max(date), .groups = "drop")
...
```{r latex-table}
#| message: false
system2("Rscript", c("scripts/generate_descriptive_table.R"))
system2("Rscript", c("scripts/generate_risk_off_table.R"))
```
...
variables in the correct Cholesky ordering. What follows is the model estimation.

### Disclosure of Replication Decisions

The paper specifies a recursively restricted VAR with a Cholesky identification
scheme, where the Risk-off variable is affected only by its own lagged values
and the other nine variables are unrestricted. Seasonal dummies and a time
dummy enter as exogenous regressors. The paper states that AIC determines the
optimal lag and that 95 percent confidence intervals are provided. The paper
does not disclose the following details.

### Seasonal dummy structure

The paper says seasonal dummies without specifying monthly or quarterly
periodicity. We use eleven monthly indicator variables with January as the
reference month. Monthly dummies are the natural reading for a daily-frequency
VAR where the lower-frequency series (WUI, REER, RGDP, capital flows) enter
through quadratic interpolation. The interpolation injects artificial
intra-period variation that monthly indicators absorb, preventing the seasonal
pattern in the source data from biasing coefficient estimates.

### Time dummy definition

The paper says a time dummy without specifying a linear trend, a break dummy,
or an intercept shift. We use a linear time trend indexed from 1 to T, which
is the simplest specification that satisfies the stated requirement.

### Maximum lag search range

The paper says AIC selects the lag but does not state the maximum searched. We
search lags 1 through 20. At lag 20 each equation estimates 200 endogenous
coefficients plus 12 exogenous coefficients. With 5,199 complete observations
in the replication period the observation to parameter ratio exceeds 24 to 1,
which is adequate for AIC to discriminate without overfitting. If AIC selects
lag 20, we flag this as a potential boundary effect.

### Confidence interval method

The paper says 95 percent confidence intervals are provided by the dotted
lines but does not specify whether they are analytical or bootstrap. We
produce both. The standard asymptotic delta method for VAR impulse response
confidence intervals propagates the coefficient covariance through the IRF
recursion via Monte Carlo simulation. This is the default in every major VAR
package (EViews, Stata, R vars) and is the most likely method the paper used.
We implement this via Monte Carlo integration: draw shocks from N(0, Sigma_u),
generate synthetic data, re-estimate the VAR, and compute IRFs for each of
500 replications. The 2.5th and 97.5th percentiles across replications form
the 95 percent band. These bands widen with the forecast horizon, reflecting
the increasing uncertainty of longer-horizon impulse responses.

The bootstrap method (residual-based, 100 replications) is added as a
robustness check. The capital flow variables in our dataset exhibit non-normal
distributions. `other_pct` has a mean of 0.39 and a maximum of 28.55 with a
standard deviation of 3.16, and the binary `risk_off` variable cannot produce
normally distributed residuals by construction. Bootstrap confidence intervals
make no distributional assumption and provide a more honest assessment of
sampling variability when normality fails.

### Restricted VAR implementation

The paper describes the block exogeneity constraint as setting the coefficients
of non-Risk-off variables to zero in the Risk-off equation. The paper does not
disclose the software package. We implement the restriction via
equation-by-equation OLS: the Risk-off equation is estimated on its own lags
and the exogenous variables, and the other nine equations are estimated with
all variables. This matches the paper stated mathematical specification
exactly.

### Forecast error variance decomposition

The working paper version (ADBI Working Paper 1345) does not mention FEVD. We
include it as a standard VAR diagnostic because it answers a question the IRFs
leave open: how much of the Risk-off shock total effect on each variable is
attributable to the shock itself versus propagation through the system. The
decomposition uses the same Cholesky ordering, so the variance allocation
inherits the same identification assumptions.

### Stationarity tests

The paper states that all endogenous variables are stationary but does not
report the test statistic, lag specification, or significance threshold. We
apply the Augmented Dickey-Fuller test with AIC lag selection and a maximum of
20 lags at the 5 percent significance level, consistent with the 95 percent
confidence intervals used throughout the paper. If any variable tests as
non-stationary we report the discrepancy.

```{r svar-analysis}
#| message: false
#| cache: true
# Output: data/processed/var_results/ (figures, CSV tables)
# Skip if outputs already exist from a prior run
if (!file.exists("data/processed/var_results/fevd_unrestricted_1999_2026.csv")) {
  system2("uv", c("run", "scripts/var_analysis.py"))
} else {
  message("SVAR outputs found, skipping re-run. Delete data/processed/var_results/ to force.")
}
```

### What the Script Produces

All outputs cover both the paper replication period (1999 to 2021) and the
extended period (1999 to 2026) for both the restricted and unrestricted VAR
specifications.

### Implementation Note

The VAR estimation runs in Python through a standalone script called by R via
`system2()`. Python code blocks embedded directly in the QMD would require the
reticulate R package, which is not installed in this environment. The existing
pipeline already uses this pattern for the quadratic interpolation step.

Keeping the computation in a separate script also avoids re-running the full
SVAR (2 to 12 minutes depending on hardware) on every render. A cache check
skips execution when output files already exist.

The full Python source is shown in a collapsible section at the bottom of this
document for readers who prefer to inspect the implementation directly.
...
#| message: false
#| echo: false
fevd_files <- list.files("data/processed/var_results", pattern = "fevd_.*csv", full.names = TRUE)
for (f in fevd_files) {
  cat("\n\nFile:", basename(f), "\n\n")
  ...
  print(knitr::kable(short, caption = basename(f), row.names = FALSE))
}
```
...
#| message: false
#| echo: false
peak_files <- list.files("data/processed/var_results", pattern = "irf_peak_.*csv", full.names = TRUE)
for (f in peak_files) {
  cat("\n\n", basename(f), "\n\n")
  df <- read.csv(f)
  print(knitr::kable(df, caption = basename(f), row.names = FALSE))
}
```

### Impulse Response Functions

The figure below overlays the restricted VAR impulse responses for the paper
replication period (1999 to 2021, blue) and the extended period (1999 to 2026,
orange). Each panel shows the response of one endogenous variable to a
one-standard-deviation structural shock on the Risk-off indicator over 125
working days. The shaded bands represent 95 percent confidence intervals.

```{r irf-period-comparison}
#| fig-height: 10
#| fig-width: 16
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/irf_period_comparison_restricted.png")
```

### Restricted VAR IRFs (Analytical CIs)

#### Paper Replication Period: 1999 to 2021

```{r irf-restricted-analytical}
#| fig-height: 10
#| fig-width: 16
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/irf_restricted_1999_2021_analytical.png")
```

#### Extended Period: 1999 to 2026

```{r irf-restricted-analytical-ext}
#| fig-height: 10
#| fig-width: 16
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/irf_restricted_1999_2026_analytical.png")
```

### Analytical vs Bootstrap CI Comparison
...
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/irf_restricted_1999_2021_ci_comparison.png")
```

#### Extended Period: 1999 to 2026

```{r irf-ci-comparison-ext}
#| fig-height: 10
#| fig-width: 16
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/irf_restricted_1999_2026_ci_comparison.png")
```

### Frequency Robustness

The restricted VAR re-estimated on weekly and monthly aggregated data
(Beirne and Sugandi 2023, Appendix 4).
...
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/irf_restricted_1999_2021_w.png")
```
...
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/irf_restricted_1999_2021_me.png")
```
...
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/irf_restricted_1999_2026_w.png")
```
...
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/irf_restricted_1999_2026_me.png")
```
...
```{r risk-off-appendix}
#| message: false
system2("Rscript", c("scripts/generate_risk_off_charts.R"))
```
...
The full VAR estimation script is shown below. It is called by the
`svar-analysis` chunk above via `system2("uv", c("run", "scripts/var_analysis.py"))`.
...
#| results: asis
cat("```python\n",
    paste(readLines("scripts/var_analysis.py"), collapse = "\n"),
    "\n```\n", sep = "")
```
