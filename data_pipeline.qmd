...
|----------|------------|-----------|-------------------|--------|
| `risk_off` | Risk-off event indicator: 1 when VIX is at least 10 points above its 60-day moving average | Daily | `VIX >= MA60(VIX) + 10` — identifies distress episodes as the VAR shock variable | Bloomberg, LSEG |
| `spread` | Yield spread between 10-year JGB and 10-year US Treasury | Daily | `JGB10Y - US10Y` — captures safe-haven bond market channel | Bloomberg, LSEG |
| `log_nikkei` | Log of Nikkei-225 stock index | Daily | Equity market response to risk-off, transmission to portfolio flows | Bloomberg, LSEG |
...
| `debtsec_pct` | Net portfolio investment inflows to debt securities (% of nominal GDP) | Monthly → daily (quadratic) | `(debt_flows_usd / GDP_usd) * 100` — portfolio debt channel | BOJ, IMF, Bloomberg, LSEG |
| `equity_pct` | Net portfolio investment inflows to equity (% of nominal GDP) | Monthly → daily (quadratic) | `(equity_flows_usd / GDP_usd) * 100` — portfolio equity channel | BOJ, IMF, Bloomberg, LSEG |
| `other_pct` | Net other investment inflows (% of nominal GDP) | Monthly → daily (quadratic) | `(other_flows_usd / GDP_usd) * 100` — banking-flow channel | BOJ, IMF, Bloomberg, LSEG |
| `direct_pct` | Net direct investment inflows (% of nominal GDP) | Monthly → daily (quadratic) | `(direct_flows_usd / GDP_usd) * 100` — FDI channel | BOJ, IMF, Bloomberg, LSEG |
...
write_csv(desc_stats, file.path(DATA_OUT, "descriptive_stats.csv"))
cat("descriptive_stats.csv written:", nrow(desc_stats), "rows\n")
```
```{r variance-test}
#| message: false
