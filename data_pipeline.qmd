...
Density plots overlay distribution shapes across the paper period, extended period, and full sample — making it easy to spot shifts in central tendency, spread, and multimodality.
...
plot_data <- plot_data |>
  left_join(full_means, by = "variable")
...
ggplot(plot_data, aes(x = value, fill = period, color = period)) +
  ...
  scale_fill_manual(values = c("#1b5e9e", "#e35f21", "#444444")) +
  scale_color_manual(values = c("#1b5e9e", "#e35f21", "#444444")) +
  labs(x = NULL, y = NULL, fill = NULL, color = NULL,
       caption = "Dashed vertical line: full-sample mean") +
...
Time series extend the paper\'s Appendix 1 figures to the full sample with period shading, making it easy to see structural shifts after 2021.
...
ggplot(ts_data, aes(x = date, y = value)) +
  geom_rect(aes(xmin = paper_end, xmax = max(date),
                ymin = -Inf, ymax = Inf),
            fill = "#e35f21", alpha = 0.04, inherit.aes = FALSE) +
  geom_line(aes(color = period), linewidth = 0.3, alpha = 0.7) +
  ...
  theme(legend.position = "bottom",
        strip.text = element_text(face = "bold"),
        panel.grid.minor = element_blank()) +
  annotate("text", x = as.Date("2010-01-01"), y = Inf,
    label = "Paper Period", hjust = 0, vjust = 1.5,
    size = 3, color = "#1b5e9e", alpha = 0.6) +
  annotate("text", x = as.Date("2023-10-01"), y = Inf,
    label = "Extended", hjust = 0, vjust = 1.5,
    size = 3, color = "#e35f21", alpha = 0.6)
```

A correlation heatmap summarizes pairwise relationships across all ten variables, useful for understanding the VAR ordering and checking for multicollinearity.
...
corrplot(cor_matrix, method = "color", type = "upper",
  ...
  col = colorRampPalette(c("#1b5e9e", "white", "#e35f21"))(200),
  addCoef.col = "black", number.cex = 0.55,
  cl.cex = 0.6, mar = c(0, 0, 1, 0),
