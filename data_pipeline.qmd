...
the price channels through which risk-off shocks transmit. The VIX itself
The VIX panel defines the episodes. Every shaded band corresponds to a VIX
spike exceeding 40, with peaks at roughly 80 during the global financial
crisis in 2008 and 85 during the initial COVID outbreak in early 2020. The
bilateral USD/JPY exchange rate panel shows the yen strengthening
sharply during every risk-off episode through 2020, dropping from 120 to
roughly 75 between 2008 and 2011. The REER panel tells the same story
in trade-weighted terms, declining from 120 in the late 1990s to roughly 70
by 2015. Both exchange rate measures then break pattern after 2021. The yen
depreciates steadily from 105 to 160 against the dollar, and the REER falls
from 80 to 50. This post-2021 divergence from the safe-haven narrative is the
single most important structural shift in the data, and it drives the
extended-period VAR results in the Discussion of Results below.

The sovereign yield spread channel The JGB and US Treasury yield panels reveal the
flight-to-safety mechanism in operation. US 10-year Treasury yields decline
sharply during each risk-off episode, from 5 percent in 2000 to 2.5 percent in
2008 to near zero in 2020. Japanese government bond yields compress even
further, falling from 2 percent in 1999 to near zero by 2010, staying there
for over a decade, then rising sharply after 2023 to roughly 2.5 percent as
the Bank of Japan began normalising policy. The paper's argument that risk-off
shocks widen the JGB-US Treasury spread through safe-haven demand for Japanese
debt is clearly visible in the raw data through 2020. After 2021 the spread
narrows not because risk-off dynamics have changed but because both yields are
rising simultaneously under fundamentally different monetary policy regimes.
...
The capital flow group The four capital flow panels confirm the paper's
observation that balance-of-payments flows are noisy and rarely show a
consistent direction during risk-off episodes. Debt portfolio flows oscillate
between sharp inflows and outflows regardless of risk-off status. Equity
portfolio flows follow a similar high-frequency pattern. Other investment
flows, the catch-all category covering short-term interbank lending and trade
credit, exhibit the highest variance, with a massive spike to nearly 29
percent of GDP in early 2020. Direct investment flows are the quietest of the
four, hovering near zero for most of the sample with occasional outliers. The
pink risk-off bands do not systematically align with the direction of any flow
series, which is itself the finding. The price channels (exchange rates and
yields) respond cleanly to risk-off episodes. The volume channels (capital
flows) do not.
