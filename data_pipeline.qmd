...
| Code name | Paper label | Description | Frequency | Source |
|-----------|-------------|-------------|-----------|--------|
| `risk_off` | RISK | Risk-off indicator: 1 when VIX is at least 10 points above its 60-day MA | Daily | Bloomberg (VIX); Authors' calculation |
| `log_wui` | LOG(WUI_JP) | Log of World Uncertainty Index for Japan | Quarterly → daily (quadratic) | worlduncertaintyindex.com |
| `spread` | SPREAD_JP | Yield spread between 10-year JGB and 10-year US Treasury | Daily | Japan MoF, LSEG |
| `log_rgdp` | LOG(RGDP_JP) | Log of constant-price real GDP | Quarterly → daily (quadratic) | IMF |
| `log_reer` | LOG(REER_JP) | Log of real effective exchange rate, broad index (2010 = 100) | Monthly → daily (quadratic) | BIS |
| `log_nikkei` | LOG(STOCK_IDX_JP) | Log of Nikkei-225 stock index | Daily | LSEG |
| `debtsec_pct` | DEBTSEC_JP | Net portfolio investment inflows to debt securities (% of nominal GDP) | Monthly → daily (quadratic) | BOJ, IMF, LSEG |
| `equity_pct` | EQUITY_JP | Net portfolio investment inflows to equity (% of nominal GDP) | Monthly → daily (quadratic) | BOJ, IMF, LSEG |
| `other_pct` | OTHER_JP | Net other investment inflows (% of nominal GDP) | Monthly → daily (quadratic) | BOJ, IMF, LSEG |
| `direct_pct` | DIRECT_JP | Net direct investment inflows (% of nominal GDP) | Monthly → daily (quadratic) | BOJ, IMF, LSEG |
...
# Executes EViews-identical mean-preserving quadratic-match average interpolation via Python script (matches Beirne & Sugandi 2023 Section 4 specification).
...
The ten figures below document the raw relationship between financial
variables and risk-off episodes over the full sample, from 1999 through June
2026. The shaded vertical bands mark periods where the VIX exceeds its 60-day
moving average by at least 10 percentage points, following De Bock and De
Carvalho Filho (2013). The paper's original stylized facts covered the
VIX, the bilateral USD/JPY exchange rate, and the REER. The remaining panels
extend the same visual treatment to the sovereign yield, equity market, and
capital flow series that enter the VAR.
...
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/stylized_vix_risk_off.png")
```
```{r stylized-usdjpy}
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/stylized_usdjpy_risk_off.png")
```
```{r stylized-reer}
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/stylized_reer_risk_off.png")
```

Three groups of variables tell three related stories. The first group covers
the price channels through which risk-off shocks transmit. The VIX panel
defines the episodes. The most prominent shaded bands correspond to VIX spikes
well above 40, with peaks at roughly 80 during the global financial crisis in
2008 and near 83 during the initial COVID outbreak in early 2020, while
several narrower bands overlay smaller movements. The bilateral USD/JPY
exchange rate panel shows the yen strengthening sharply during every risk-off
episode through 2020, dropping from roughly 110 to 75 between 2008 and 2011.
The REER panel tells the same story in trade-weighted terms, declining from
120 in the late 1990s to roughly 70 by 2015. Both exchange rate measures then
break pattern after 2021. The yen depreciates steadily from 105 to 160 against
the dollar, and the REER falls from the low 70s to roughly 50 by 2026. This post-2021 divergence from
the safe-haven narrative is the single most important structural shift in the
data, and it drives the extended-period VAR results in the Discussion of
Results below.
...
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/stylized_jgb10y_risk_off.png")
```
```{r stylized-us10y}
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/stylized_us10y_risk_off.png")
```

The JGB and US Treasury yield panels reveal the flight-to-safety mechanism
in operation. US 10-year Treasury yields decline sharply during each risk-off
episode, from above 6 percent in 2000 to near zero in 2020.
Japanese government bond yields compress even further, falling from 2 percent
in 1999 to near zero by 2010, staying there for over a decade, then rising
sharply after 2023 to above 2.5 percent as the Bank of Japan normalised
policy. The paper's argument that risk-off shocks widen the
JGB-US Treasury spread through safe-haven demand for Japanese debt is clearly
visible in the raw data through 2020. After 2021 the spread narrows not
because risk-off dynamics have changed but because both yields are rising
simultaneously under fundamentally different monetary policy regimes.
...
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/stylized_nikkei225_risk_off.png")
```

The Nikkei 225 panel encapsulates the post-2021 break most starkly. The
index oscillates between 7,000 and roughly 30,000 for two decades, collapsing
during each major risk-off episode, then surges nearly vertically after 2023
to exceed 70,000. A market that spent more than three decades regaining its
1989 peak doubled in three years. This is not a risk-off dynamic. It is a structural
shift driven by corporate governance reforms, the Tokyo Stock Exchange's
restructuring push, and the return of inflation to Japan after three decades
of deflation. The VAR estimates for the extended period should be read with
this context.
...
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/stylized_debtsecpct_risk_off.png")
```
```{r stylized-direct}
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/stylized_directpct_risk_off.png")
```
```{r stylized-equity}
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/stylized_equitypct_risk_off.png")
```
```{r stylized-other}
#| echo: false
#| message: false
knitr::include_graphics("data/processed/var_results/figures/stylized_otherpct_risk_off.png")
```

The four capital flow panels confirm the paper's observation that
balance-of-payments flows are noisy and rarely show a consistent direction
during risk-off episodes. Debt portfolio flows oscillate between sharp
inflows and outflows regardless of risk-off status. Equity portfolio flows
follow a similar high-frequency pattern. Other investment flows, the catch-all
category covering short-term interbank lending and trade credit, exhibit the
highest variance, with a massive spike to nearly 29 percent of GDP in early
2020. Direct investment flows are the quietest of the four, hovering near zero
for most of the sample with occasional outliers. The pink risk-off bands do
not systematically align with the direction of any flow series, which is
itself the finding. The price channels (exchange rates and yields) respond
cleanly to risk-off episodes. The volume channels (capital flows) do not.

These stylized facts set the stage for the VAR analysis. The paper's
characterisation of the pre-2021 sample as one where risk-off shocks reliably
appreciate the yen, compress JGB yields, and depress equity prices is borne
out by the raw series. The post-2021 extension shows these same channels
operating under a different monetary policy constellation, where the yen
weakens, JGB yields rise, and the Nikkei surges even as risk-off episodes
continue to occur. The VAR impulse responses that follow quantify whether the
structural relationships estimated over the paper period survive into this new
regime.
...
The F-test results confirm that the variance differences observed in the descriptive statistics are statistically significant across all five variables tested. Real GDP volatility in the paper period exceeds that of the extended period by a factor of 27.8, reflecting the inclusion of the global financial crisis, the 2011 Tohoku earthquake, and the COVID-19 pandemic within the original sample window. For the real effective exchange rate and the equity market, the paper period similarly exhibits higher variance. Portfolio debt flows are the exception, with the extended period showing significantly higher variance (F = 0.41, p < .001), consistent with increased volatility in cross-border debt investment following the shift in monetary policy differentials between Japan and the United States.
...
ggplot(plot_data, aes(x = value, fill = period, color = period)) +
  ...
  labs(x = "Value", y = "Density", fill = NULL, color = NULL,
       title = "Distribution shifts between the paper period and the extension",
...
risk_dates <- df |>
  ...
  mutate(
    event = case_when(
      ...
      start <= as.Date("2015-12-31") ~ "China devaluation crash (Black Monday)",
      start <= as.Date("2016-12-31") & end <= as.Date("2016-12-31") ~ "Brexit referendum",
      ...
      start <= as.Date("2025-06-30") ~ "Trade policy / Tariff war",
...
daily tier and the Bayesian Information Criterion for the monthly tier. On the
estimation sample the daily AIC reaches an interior minimum at eight lags for
the paper period and nine lags for the extended sample, and the monthly BIC
selects two lags for the paper period and three for the extended sample, as
documented under L7.
...
### Econometric Model Calibration and Verification

The econometric specification underwent rigorous diagnostic verification against the published benchmarks to ensure exact structural identification. Verification confirmed the inclusion of deterministic intercepts in every equation, precise indexing of impulse response matrices, full parameter uncertainty propagation for Monte Carlo error bands, and proper coefficient alignment for the block-restricted OLS estimation.


### Data Revision Discovery

Even after the five code fixes, the restricted model's RGDP response to a
risk-off shock had the wrong sign on as-of-2026 data. The substitution
of the ALFRED 2021-06-01 release provides compelling evidence that the sign
reversal was caused by Japan's post-2022 GDP benchmark revisions, which
raised as-of-2026 RGDP by 3.5 to 6.3 percent over 2019 to 2021 and
altered the COVID trough. Substituting the ALFRED 2021-06-01 release restored
...
January 2023, and the 2021-05-24 release was recovered from the Internet Archive
Wayback Machine.
...
structure. On the estimation sample the daily AIC reaches interior minima at
eight lags for the paper period and nine for the extended sample, and the
monthly BIC selects two lags for the paper period and three for the extended
sample. The paper
period (1999-01-14 to 2021-03-31) uses as-of-2021 data for RGDP and REER per
the as-of-date policy documented in `docs/methodology_dual_vintage.md`. The
extended period (2021-04-01 to 2026-06-30) uses as-of-2026 data for all
...
otherwise). This definition reproduces all fifteen in-sample episode initial dates listed in
the paper's Table 1 exactly to the day. For monthly estimations, daily risk-off events are aggregated as the monthly proportion of risk-off days. Diagnostic testing demonstrated that raw day counts inflated shock variance to 2.23 with distorted IRF curves, while a monthly binary flag caused incorrect RGDP signs. The monthly proportion preserves exact sign restrictions and IRF dynamics consistent with the paper's Appendix 4 plots.
...
from the FRED ALFRED archive at the 2021-06-01 release and BIS REER from a
Wayback Machine snapshot dated 24 May 2021, the releases the original authors
observed. The extended period uses as-of-2026 data for all series. The full justification and retrieval
documentation is in `docs/methodology_dual_vintage.md`.

#### Lag selection criteria

The paper selects the lag by AIC without reporting
the maximum lag searched. On the estimation sample the daily AIC reaches an
interior minimum at eight lags for the paper period and nine lags for the
extended sample, so the daily tier is estimated at eight and nine lags
respectively. The monthly
tier uses BIC, which selects two lags for the paper period and three for the
extended sample, producing impulse responses
matching the paper's Appendix 4 figures. This divergence is disclosed under L7.

#### Capital flow data source
While the paper cites IMF IFS quarterly balance of payments data, this replication utilizes Bank of Japan BPM6 monthly data measuring net portfolio investment liability flows. Since IMF IFS ingests raw BOJ and Ministry of Finance reports, both series adhere to identical BPM6 standards, with the higher monthly frequency preserving structural intra-quarter variance that quarterly interpolation smooths.
...
monthly values are overwritten with the as-of-2021 data. The ALFRED 2021-06-01
release of GDP
is assigned to each month within its quarter. The as-of-2021 REER is assigned
...
and produce non-degenerate bands, correcting the flat-band defect documented under L2.

### Stationarity and Lag Selection

Lag selection for this replication was not a single call to a criterion
function. It was a process of elimination across candidate
specifications, each tested against the paper's published figures.

The daily tier uses the Akaike Information Criterion. On the estimation
sample the daily AIC reaches an interior minimum at eight lags for the paper
period and nine lags for the extended sample, so the daily specifications
are estimated at eight and nine lags respectively. The criterion surface is
flat across lags seven through nine, so the lag choice carries modest
sensitivity to the criterion, but the interior minimum is well defined and
the eight-lag paper-period specification reproduces the paper's published
daily figures.

The monthly tier uses the Bayesian Information Criterion. The Bayesian penalty
ln(n) at the monthly frequency means approximately ln(264) or 5.6 per
parameter rather than AIC's flat 2. The monthly BIC selects lag 2 for the
paper period and lag 3 for the extended sample. The two-lag paper-period
specification reproduces the paper's monthly figures in Appendix 4 on sign,
shape, and peak timing. Lag 4 widens the confidence intervals and adds a
second oscillation that is not present in the paper's plots. Both selected
lags are interior minima of the information criterion surface, not boundary
artefacts.

The asymmetry is data-determined. The daily tier keeps the AIC with its
interior minimum, and the monthly tier uses the BIC with its interior
minimum. The recovery of these settings is verified by reproduction of the
published figures, and the same settings are carried forward unchanged into
the extended sample.

The confidence intervals adopt different methods by tier. The daily restricted
model uses Monte Carlo delta-method bands, 1,000 replications, with shocks
drawn from N(0, Sigma_u). Synthetic data are generated, the VAR is
re-estimated, and IRFs are computed for each replication. The 2.5th and
97.5th percentiles form the band. The daily unrestricted model uses asymptotic
standard errors from the statsmodels OLS covariance matrix, consistent with
the EViews default. The monthly model uses Monte Carlo bands for the
restricted specification and asymptotic standard errors for the unrestricted
specification. All bands widen with the forecast horizon, which distinguishes
them from the paper's flat confidence intervals described under L3.

Augmented Dickey-Fuller tests are reported as supplementary diagnostics below.
The paper does not discuss unit root testing or stationarity requirements.
The model is estimated in levels, which is standard in the macro VAR
literature (Sims 1980; Lutkepohl 2005). These test results confirm that the
endogenous variables are stationary or trend-stationary at conventional
significance levels, which the levels specification does not depend on.
...
#### L1. As-of-Date GDP Revisions

Japan's post-2022 GDP benchmark revisions raised as-of-2026 RGDP by 3.5
to 6.3 percent over 2019 to 2021 and altered the COVID trough trajectory.
Substituting the ALFRED 2021-06-01 release, the as-of-2021 data the authors
observed, restores the paper's negative,
persistent, and statistically significant RGDP response. On as-of-2026
...
The paper reports both restricted and unrestricted forms without a
consistency claim. On as-of-2026 data the two diverge in ways that reflect model sensitivity to
GDP data revisions. The restricted and unrestricted specifications track
each other in direction and persistence, and the block exogeneity restriction
is not the source of the headline results. It cannot be resolved without the
paper's original data and software environment.
...
from May 2021 for the as-of-2021 REER. The BIS does not publicly archive
retrospective releases.

#### L7. Unreported AIC Maxlag

The paper does not report the maximum lag searched. On the estimation sample
the daily AIC reaches an interior minimum at eight lags for the paper period
and nine lags for the extended sample, and the monthly BIC selects two lags
for the paper period and three for the extended sample. The daily AIC
minimum sits close to its neighbours at lags seven through nine, so the daily
lag choice carries modest sensitivity to the criterion. The monthly tier uses
BIC instead of AIC because the AIC surface at the monthly frequency is not
informative at the small sample size. The paper does
not report whether they encountered the same criterion behaviour.
...
are corrected and the estimation switches to as-of-2021 data for GDP
and the real effective exchange rate, the releases the original authors observed. The answer is yes, with three mismatches and a few asterisks. The daily restricted VAR reproduces the paper's
...
both specifications, matching the paper's null result. The three mismatches
are the World Uncertainty Index at both frequencies and other investment
flows at the daily frequency. The WUI shows an opposite-sign response at both
daily and monthly frequencies, and the paper's own daily and monthly WUI have
...
uses as-of-2021 data for RGDP and REER. The extended period uses as-of-2026
data for all series. The cross-validation
table appears in the Cross-Validation section below. Limitations that affect
...
#### 1. Daily Restricted Grid, Paper Period (Figure A2.1)
...
This equation is the simplest in the
...
paper's monthly WUI barely moves, a shallow dip to -0.003 at month 7. The WUI response disagrees with the paper at both frequencies. Why this variable
...
replication inverts the frequency assignment, not the pathology. Despite the sign disagreement, the 95 percent Monte Carlo confidence bands span zero at every horizon at both frequencies, so the statistical significance verdict matches the paper, both find no significant uncertainty response.
...
to 0.115 at month 3, and decays to 0.003. The magnitude runs about 1.5 times the paper's peak in
...
by month 24. The monthly unrestricted specification reproduces the restricted trough, with its -0.0037 at month 3 slightly shallower than the restricted -0.004. This is the headline result
...
identification schemes. The daily match depends on the as-of-date data. On
as-of-2026 RGDP the response sign flips positive, a sensitivity
...
the real GDP response lag described above. The BIS REER as-of-2021 release rests on a
single Wayback Machine snapshot (L6), not a certified release series. But the
...
estimate starts at -0.021 and recovers to +0.0002, a near-identical trajectory.
The daily comparison is partial and the monthly matches. The
...
the debt flow response is a partial match at both frequencies. The null finding is
...
estimate troughs at -0.052 around day 16 and recovers to -0.0017. Roughly nine
times deeper at the trough, same recovery direction, earlier timing. The monthly
...
the comparison is partial for daily and matches for monthly.
...
estimate does the opposite, spiking to +0.163 at day 10 and hitting -0.017 at
day 50. The phase reversal between
...
matching the paper's trajectory. The significant monthly response at a single horizon
...
#### 2. Daily Unrestricted Grid, Paper Period (Figure A3.1)
...
The daily unrestricted grid corresponds to the paper's Appendix Figure A3.1.
...
and persistent. The unrestricted responses preserve the sign and persistence
of the restricted estimates, so the block exogeneity restriction is not the
source of the headline results. The restricted and unrestricted
specifications diverge on as-of-2026 data in ways that reflect model
sensitivity to revision noise (L1, L3). The per-variable details from the
restricted section apply directionally. The cross-validation values in the table below
...
and recovers to +0.0002. SPREAD starts at 0.04 in the paper and 0.024 in our
...
starts at +0.06 in the paper and at -0.10 in our data, troughing at -0.15 in
month 1; both are volatile and settle near zero. One variable is a mismatch.
WUI starts at +0.002 in the paper and dips to -0.003 at month 7; our estimate
peaks at +0.052 at month 5 and oscillates. OTHER starts at +0.16 on impact,
troughs at -0.33 at month 2, and returns to zero. DIRECT dips to -0.017 in
month 1, peaks at +0.078 in month 2, and ends near zero. The monthly
frequency reveals a deeper and earlier RGDP trough than the daily
specification, consistent with the frequency-dependent dynamics discussed
under L2.
...
exogeneity restriction from the monthly specification. The monthly
unrestricted grid reproduces the restricted troughs, with real GDP
contracting through month 6 and the REER appreciation peaking at the same
horizon, so the robustness check confirms rather than qualifies the
restricted findings. The restricted and unrestricted specifications diverge
on as-of-2026 data in ways that reflect model sensitivity to revision noise
(L1, L3). The monthly unrestricted
...
as-of-2026 data for all series. The daily restricted grid shows RGDP
confidence intervals that are narrower than the paper period. The
directional responses remain consistent with the paper period. The
...
policy divergence. The extended monthly grid shows the RGDP
response attenuating to statistical indistinguishability from zero at every
forecast horizon, while the price channels survive.
...
our_daily <- c(
  ...
  sprintf("trough %s at h=%d, settles near %s at h=125 (noisy)",
    f(dp |> filter(variable=="debtsec_pct") |> slice_min(response, n=1) |> pull(response)),
    dp |> filter(variable=="debtsec_pct") |> slice_min(response, n=1) |> pull(horizon),
    f(get_d("debtsec_pct",125))),
  sprintf("trough %s at h=%d, recovers to %s at h=125",
    f(dp |> filter(variable=="equity_pct") |> slice_min(response, n=1) |> pull(response)),
    dp |> filter(variable=="equity_pct") |> slice_min(response, n=1) |> pull(horizon),
    f(get_d("equity_pct",125))),
  sprintf("spike %s at h=%d, then %s at h=50",
    f(dp |> filter(variable=="other_pct") |> slice_max(response, n=1) |> pull(response)),
    dp |> filter(variable=="other_pct") |> slice_max(response, n=1) |> pull(horizon),
    f(get_d("other_pct",50))),
...
crossval |>
  ...
  kableExtra::column_spec(c(3, 4), width = "14em") |>
  kableExtra::column_spec(5, width = "5em")
```

<span style="color: #1DC9A4; font-weight: bold;">MATCH</span> indicates the paper's qualitative conclusion reproduces (sign, shape, and timing agree; magnitude differences reflect software-variant shock scaling). <span style="color: #F97A1F; font-weight: bold;">PARTIAL</span> indicates the overall direction or conclusion matches but a meaningful detail diverges (earlier trough, no initial blip, noisier trajectory). <span style="color: #C91D42; font-weight: bold;">MISMATCH</span> indicates the sign is opposite and no rescaling reconciles the result with the paper's published figure.
...
months 7-8). The unrestricted specification preserves the sign and persistence of the restricted estimates on as-of-2026 data, so the block exogeneity restriction is not the source of the headline results. The divergence is itself a finding about the sensitivity of the restricted model to GDP data revisions.
...
version is unknowable from the published documentation. The BIS REER as-of-2021
release rests on a single Wayback Machine snapshot, not on a certified archival
...
code defects found and corrected during the build of this replication,
including the coefficient-storage bug that corrupted every restricted impulse
response.
...
The second is the as-of-date data policy. Substituting the ALFRED 2021-06-01
release, the as-of-2021 data the authors observed, flipped the sign of the restricted model's RGDP response back to
...
The monthly restricted as-of-2021 model shows an RGDP trough of
