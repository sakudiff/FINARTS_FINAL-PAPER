...
Python (see `scripts/quadratic_interpolate.py`), not by running EViews
...
regardless of information content (see `docs/logs/2026-07-22-svar-issues-
***REMOVED***`). The daily tier retains AIC per the paper's method. The paper
...
### Python Source Code (Deprecated Script)

The script below is the original daily VAR estimation code (`var_analysis.py`)
that contained the IRF transpose bug and was superseded by the two-tier
results. It is retained for transparency. The two-tier results in
`data/processed/var_results/twotier/` were generated by an independent Python
estimation with the corrected IRF recursion, vintage GDP, and Monte Carlo
confidence bands.
