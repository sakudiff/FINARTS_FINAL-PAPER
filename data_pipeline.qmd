...
| Variable | Description | Frequency | Formula / Purpose | Source |
|----------|------------|-----------|-------------------|--------|
| `risk_off` | Risk-off event indicator: 1 when VIX is at least 10 points above its 60-day moving average | Daily | `VIX >= MA60(VIX) + 10` — identifies distress episodes as the VAR shock variable | Authors' calculation from Bloomberg VIX data |
| `spread` | Yield spread between 10-year JGB and 10-year US Treasury | Daily | `JGB10Y - US10Y` — captures safe-haven bond market channel | Bloomberg |
| `log_nikkei` | Log of Nikkei-225 stock index | Daily | Equity market response to risk-off, transmission to portfolio flows | Bloomberg |
| `log_reer` | Log of real effective exchange rate, broad index (2010 = 100) | Monthly → daily (quadratic) | Exchange rate competitiveness channel, safe-haven appreciation | BIS |
| `log_rgdp` | Log of constant-price real GDP | Quarterly → daily (quadratic) | Real spillover channel: output response to risk-off | IMF |
| `log_wui` | Log of World Uncertainty Index for Japan | Quarterly → daily (quadratic) | Economic policy uncertainty transmission | worlduncertaintyindex.com |
| `debtsec_pct` | Net portfolio investment inflows to debt securities (% of nominal GDP) | Monthly → daily (quadratic) | `(debt_flows_usd / GDP_usd) * 100` — portfolio debt channel | BOJ, IMF, Bloomberg |
| `equity_pct` | Net portfolio investment inflows to equity (% of nominal GDP) | Monthly → daily (quadratic) | `(equity_flows_usd / GDP_usd) * 100` — portfolio equity channel | BOJ, IMF |
| `other_pct` | Net other investment inflows (% of nominal GDP) | Monthly → daily (quadratic) | `(other_flows_usd / GDP_usd) * 100` — banking-flow channel | BOJ, IMF, Bloomberg |
| `direct_pct` | Net direct investment inflows (% of nominal GDP) | Monthly → daily (quadratic) | `(direct_flows_usd / GDP_usd) * 100` — FDI channel | BOJ, IMF, Bloomberg |
...
The rolling standard deviation plots reveal a sharp increase in equity market volatility beginning in early 2022, coinciding with the onset of aggressive US monetary tightening and the subsequent yen depreciation. The 260-day rolling window captures the persistence of this elevated volatility regime rather than a transient spike. Spread volatility shows a more gradual increase beginning in late 2022, reflecting the growing divergence between US and Japanese monetary policy trajectories. REER volatility remains comparatively stable across the full sample, suggesting that the exchange rate channel, while moving to a new level, did not experience a structural increase in day-to-day variance.
...
# theme_quant() and period_colors defined in setup chunk
```
...
ggplot(plot_data, aes(x = value, fill = period, color = period)) +
  ...
  labs(x = "Value", y = "Density", fill = NULL, color = NULL,
       ...
       caption = "Dashed line: full-sample mean. Variables are described in Section 4.") +
  theme_quant()
```
...
ggplot(full_dens, aes(x = value)) +
  ...
  labs(x = "Value", y = "Density",
       ...
       caption = "Values are in percentage points or natural logs per Table 2.") +
  theme_quant()
```

### Structural breaks in the extended sample

The time-series plots provide a direct visual comparison of the paper period (blue) and the extended period (orange), separated by the vertical line marking 31 March 2021. A shaded region highlights the 2022 yen depreciation episode. Notable structural shifts include the widening of the yield spread as the Federal Reserve raised rates while the BOJ maintained its yield curve control framework, and the increased amplitude of portfolio debt and equity flows after 2021.
...
ggplot(ts_data, aes(x = date, y = value)) +
  ...
  labs(x = "Year", y = NULL, color = NULL,
       ...
       caption = "Vertical line: paper period end (31 March 2021). Variables follow the Cholesky ordering.") +
