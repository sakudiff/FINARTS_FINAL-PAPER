...
rolling_vol <- final_dataset |>
  ...
  group_by(variable) |>
  mutate(
    rolling_sd = rollapply(value, width = 260, FUN = sd, fill = NA, align = "right")
  ) |>
  ungroup()
...
ggplot(rolling_vol, aes(x = date, y = rolling_sd, color = variable)) +
  ...
  annotate("text", x = as.Date("2021-03-31"), y = 0.02, label = "Paper Period End",
           hjust = 1.1, size = 3, angle = 90, color = "gray50") +
