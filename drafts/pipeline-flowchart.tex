...
% Annotation text (placed safely below importd)
\node[below=of importd, yshift=-2mm,
      font=\scriptsize\itshape, text width=5.2cm, align=left,
      text=gray!60!black] {
  Re-reads raw macro, BOJ API, and USDJPY files (no START\_DATE filter)\newline
  plus uses already-parsed 6pi-1 dataframes from step above.
};
...
\node[draw=gray!60, fill=white, rounded corners,
  ...
  \begin{tabular}{ll}
    ...
    \raisebox{1pt}{\tikz{\draw[densely dashed,->,>=Stealth] (0,0) -- (0.4,0);}} & Unfiltered data branch (see annotation below)
  \end{tabular}
};
\end{scope}
