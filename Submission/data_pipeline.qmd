...
structure. The monthly tier adopts BIC over AIC to resolve boundary-lag overfitting, as AIC selects the maximum lag cap irrespective of information criteria due to the large sample size. The daily tier retains AIC to strictly adhere to the paper's primary baseline specification. The paper
...
function. It was a process of elimination across four candidate
specifications, each eliminated by failure mode.

The first attempt used AIC for both the daily and monthly tiers, following
the paper's stated method. The daily AIC declined through the search range, with small local increases at the upper lags, through every
lag from 1 through 20 and selected the boundary. Extending the search to 30
lags produced the same behaviour. The penalty of 2 per parameter is
meaningless in a ten-variable system with over 5,000 observations. Each
additional lag adds 100 endogenous coefficients. The likelihood improvement
from absorbing a small amount of residual autocorrelation swamps the AIC
penalty at any reasonable lag cap. The criterion fails to identify a local
minimum because none exists within the searchable range.

The second attempt applied AIC to the monthly tier. The same failure pattern
appeared. The monthly AIC declines from lag 1 to lag 12, the search boundary,
on current-vintage data. The paper does not state which vintage it used, but
the boundary-selection behaviour is identical to the daily case and for the
same reason.

The third attempt switched the monthly tier to BIC. The Bayesian penalty
ln(n) at the monthly frequency means approximately ln(300) or 5.7 per
...
The fourth attempt tried BIC on the daily tier. It selected one or two lags
depending on whether the maximum searched was 20 or 30. The resulting impulse
responses were too persistent and did not reproduce the paper's daily decay
rates. BIC underpenalises autocorrelation removal because its observation
count at the daily frequency (n approximately 5,200) is an order of magnitude
larger than monthly, but the lag penalty is logarithmic, so the damage is the
same and the loss of explanatory power is visible.

An asymmetric lag selection protocol is implemented. The daily tier retains AIC with a 20-lag cap following the paper's baseline method, whereas the monthly tier adopts BIC with a 12-lag cap, selecting lag 3 across both sample periods to prevent boundary-lag distortion.
...
consistency claim. On current-vintage data the two diverge. This divergence is visible
